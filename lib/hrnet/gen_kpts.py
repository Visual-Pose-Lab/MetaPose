...
from lib.hrnet.libs.utils2d.utilitys import plot_keypoint, PreProcess, write, load_json
from lib.hrnet.libs.config import cfg, update_config
from lib.hrnet.libs.utils2d.transforms import *
from lib.hrnet.libs.utils2d.inference import get_final_preds
from lib.hrnet.libs.models import pose_hrnet
...
def gen_video_kpts(video, det_dim=416, num_peroson=1, gen_output=False):
    ...
    for ii in tqdm(range(video_length)):
        ...
        with torch.no_grad():
            ...
            ceter, scale = np.asarray(center), np.asarray(scale)
            inputs = inputs[:, [2, 1, 0]]
        ...
        center_result.append(center[0])
        scale_result.append(scale[0])

    keypoints = np.array(kpts_result)
    scores = np.array(scores_result)
    centers = np.array(center_result)
    scales = np.array(scale_result)*1.2

    # import pdb;pdb.set_trace()
    keypoints = keypoints.transpose(1, 0, 2, 3)  # (T, M, N, 2) --> (M, T, N, 2)
    scores = scores.transpose(1, 0, 2)  # (T, M, N) --> (M, T, N)

    return keypoints, scores, centers, scales
